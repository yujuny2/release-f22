...
{
    // your code here
    InorderTraversal<T> iter(root);
    std::vector<T> elements;
    for (auto i = iter.begin(); i != iter.end(); ++i) {
        elements.push_back((*i)->elem);
    }
...
bool BinaryTree<T>::isOrderedRecursive(Node* subRoot, T min, T max) const {
    ...
    return isOrderedRecursive(subRoot->right, subRoot->elem, max) && isOrderedRecursive(subRoot->left, min, subRoot->elem);
}